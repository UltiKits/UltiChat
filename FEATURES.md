...
| ultichat.config.channels.channels.channels | The channel definitions map — one entry per channel, each `{display-name, permission, range, cross-world}` plus an optional `format`. When channels are enabled, a channel's `format` replaces the chat line for its members, with `{display}` replaced by the channel's colorized display name alongside `{player}`, `{displayname}` and `{message}`; a channel without one gets the global `chat.yml` `chat.format` with its display name in front. A `format` byte-for-byte equal to one of the three strings earlier versions shipped (`{display}&f: {message}`, `{display}&7: {message}`, `&c[Staff] &f{player}&7: {message}`) counts as unset, so an upgraded server's line is unchanged; a one-character change makes it apply. The shipped channels set no `format` (`UltiKits/UltiChat#16`) | config | `config/channels.yml: channels.channels (default: 3 entries — global, local, staff, none with a format)` | n/a | n/a | admin | detailed | ChannelService#getChannelFormat, ChatListener#applyChatFormat |
...
| ultichat.config.chat.anti-spam.duplicate-window | Duplicate-detection time window, in seconds, range 0-600. `0`, the declared default, means no time limit: every retained copy counts toward `anti-spam.max-duplicate` however old, exactly the count-only detection of versions before 6.3.0, which never read this key. A positive value is a window: a retained copy sent longer ago than that stops counting, so a repeat is refused only when enough identical copies fall inside it. An upgraded server keeps the value earlier versions wrote into its file (60), which now takes effect and is announced at load (`ultichat.lifecycle.duplicate-window-warning`). `UltiKits/UltiChat#14` | config | `config/chat.yml: anti-spam.duplicate-window (default: 0 = no time limit)` | n/a | n/a | admin | brief | AntiSpamService#isDuplicate |
