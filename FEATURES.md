...
| ultichat.autoreply.persistence | A rule added, keyword-changed, or removed via the `/uchat autoreply` commands is written to `config/autoreply.yml` before the command reports success, so it survives `/uchat reload` and `/ul reload` (both re-`init()` `AutoReplyConfig` straight from disk) as well as a clean server stop. A call that changes no rule writes nothing. If the write fails, the in-memory rule set is restored to exactly what it was -- same rules, same order, same rule map instances -- the sender is told the rule could not be saved (`autoreply_save_failed`) rather than that it succeeded, and the cause is logged; no half-applied rule is left behind (UltiKits/UltiChat#17) | persistence | add/change/remove a rule via the `/uchat autoreply` commands, then run `/uchat reload` | n/a | n/a | admin | detailed | AutoReplyService#addRule, AutoReplyService#setKeyword, AutoReplyService#removeRule, ChatAdminCommands#reportSaveFailure |
