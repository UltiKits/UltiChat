...
this module's configuration files — reached by `/uchat reload` and by `/ul reload`) run three
upgrade checks: the removed-key check, the duplicate-window notice, and the channel-format notice. Deleting a `@ConfigEntry` stops the framework writing that key into a fresh file
